...
export default function ShiftsPage() {
  ...
  return (
    <div>
      <Breadcrumbs items={[{ label: t('nav.hr') }, { label: t('attendance.shifts') }]} />
      <PageHeader title={t('attendance.shifts')} subtitle={t('shifts.subtitle')} actions={<PrintButton />} />
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className="flex bg-gray-100 rounded-lg p-1">
          <button onClick={() => setTab('shifts')} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${tab === 'shifts' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}>
            <Clock className="w-4 h-4 inline ml-1" />{t('attendance.shifts')}
          </button>
          <button onClick={() => setTab('assignments')} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${tab === 'assignments' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}>
            <CalendarDays className="w-4 h-4 inline ml-1" />{t('shifts.assignments')}
          </button>
        </div>
      </div>
        ...
        <>
          <div className="mb-4">
            <button onClick={() => { setEditShiftId(null); setShiftForm(defaultShiftForm); setShowShiftModal(true); }} className="btn-primary flex items-center gap-2">
              <Plus className="w-4 h-4" /> {t('shifts.add_shift')}
            </button>
          </div>
          <DataTable
            columns={[
              { key: 'name', label: t('shifts.name') },
              { key: 'start_time', label: t('shifts.start_time') },
              { key: 'end_time', label: t('shifts.end_time') },
              { key: 'grace_minutes', label: t('shifts.grace_minutes'), render: (v) => `${v} ${t('attendance_report.minute')}` },
          ...
          <Modal isOpen={showShiftModal} onClose={() => setShowShiftModal(false)} title={editShiftId ? t('shifts.edit_shift') : t('shifts.add_shift')}>
            <div className="space-y-4">
              <div>
                <label className="block text-sm text-gray-600 mb-1">{t('shifts.name')}</label>
              ...
              <div>
                <label className="block text-sm text-gray-600 mb-1">{t('shifts.grace_minutes')}</label>
      ...
      {tab === 'assignments' && (
        <>
          ...
          <DataTable
            columns={[
              { key: 'user_name', label: t('shifts.employee') },
              { key: 'shift_name', label: t('shifts.shift') },
              { key: 'start_date', label: t('shifts.start_date') },
              { key: 'end_date', label: t('shifts.end_date_optional'), render: (v) => v || '—' },
              { key: 'is_active', label: t('common.status'), render: (v) => v ? <span className="badge badge-success">{t('common.active')}</span> : <span className="badge badge-danger">{t('common.inactive')}</span> },
              {
                key: 'actions', label: '', render: (_, row) => (
                  <div className="flex items-center gap-2">
          ...
          <Modal isOpen={showAssignmentModal} onClose={() => setShowAssignmentModal(false)} title={editAssignmentId ? t('shifts.edit_assignment') : t('shifts.add_assignment')}>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-gray-600 mb-1">{t('shifts.start_date')}</label>
      ...
      <ConfirmDialog
        ...
        message={t('shifts.confirm_delete_message')}
        variant="danger"
      />
    </div>
  );
