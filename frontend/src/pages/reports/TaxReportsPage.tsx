...
export default function TaxReportsPage() {
  ...
  const saleColumns = [
    { key: 'invoice_number', label: t('tax_report.invoice_number'), render: (v: string) => <span className="font-medium">{v || '-'}</span> },
    { key: 'invoice_date', label: t('common.date'), render: (v: string) => v ? formatDate(v) : '-' },
    { key: 'client_name', label: t('tax_report.client') },
  ...
  const purchaseColumns = [
    { key: 'invoice_number', label: t('tax_report.invoice_number'), render: (v: string) => <span className="font-medium">{v || '-'}</span> },
    { key: 'invoice_date', label: t('common.date'), render: (v: string) => v ? formatDate(v) : '-' },
    { key: 'supplier_name', label: t('tax_report.supplier') },
    { key: 'subtotal', label: t('tax_report.subtotal'), render: (v: number) => <span className="font-mono">{formatCurrency(v || 0)}</span> },
    { key: 'tax', label: t('tax_report.tax'), render: (_: any, row: any) => <span className="font-mono text-red-600">{formatCurrency(row.tax || row.vat || 0)}</span> },
    { key: 'total', label: t('tax_report.total_with_tax'), render: (v: number) => <span className="font-mono font-bold">{formatCurrency(v || 0)}</span> },
  ];
  ...
  return (
    <div>
      <Breadcrumbs items={[{ label: t('reports.title'), path: '/reports' }, { label: t('reports.tax') }]} />
      ...
      <div className="flex items-center gap-4 mb-6">
        <div><label className="block text-sm font-medium mb-1">{t('tax_report.from')}</label><input type="date" value={from} onChange={e => setFrom(e.target.value)} className="input-field" /></div>
        <div><label className="block text-sm font-medium mb-1">{t('tax_report.to')}</label><input type="date" value={to} onChange={e => setTo(e.target.value)} className="input-field" /></div>
      </div>
