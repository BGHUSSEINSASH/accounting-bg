...
router.get('/sales/:id', authenticate, (req: AuthRequest, res: Response) => {
  try {
    const db = getDatabase();
    const invoice = db.prepare(`
      SELECT si.*, c.name as client_name, c.phone as client_phone, c.tax_number as client_tax,
             u.full_name as sales_rep_name
      FROM sales_invoices si
      LEFT JOIN clients c ON c.id = si.client_id
      LEFT JOIN users u ON u.id = si.sales_rep_id
      WHERE si.id = ?
    ...
    const company = db.prepare('SELECT * FROM company_info LIMIT 1').get() as any;
  ...
  try {
    ...
    if (columns && data && data.length > 0) {
      ...
      columns.forEach((col: string) => {
        doc.text(col, 50 + (columns.indexOf(col) * colWidth), doc.y, { width: colWidth, continued: true });
      });
      doc.moveDown(0.5);
      ...
      data.forEach((row: any) => {
        const y = doc.y;
        columns.forEach((col: string) => {
          doc.text(String(row[col] || ''), 50 + (columns.indexOf(col) * colWidth), y, { width: colWidth, continued: true });
        });
        doc.moveDown(0.3);
      });
    } else {
